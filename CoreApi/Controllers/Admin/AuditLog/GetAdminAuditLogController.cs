    ...
    public class GetAdminAuditLogController : BaseController
    {
                                                      ...
                                                      [FromQuery] string search_term = default)
        {
            ...
            try {
                ...
                int totalSize = 0;
                List<AdminAuditLog> logs = default;
                (logs, totalSize) = await __AdminAuditLogManagement.GetAuditLogs(start, size, search_term);
                ...
                LogInformation($"Get admin auditlog success, user_name: { user.UserName }, start: { start }, size: { size }, search_term: { search_term }");
