...
namespace CoreApi.Controllers.Admin.Role
{
    ...
    public class NewSocialRoleController : BaseController
    {
        public NewSocialRoleController(BaseConfig _BaseConfig) : base(_BaseConfig, true)
        {
        }

        [HttpPost("")]
        public async Task<IActionResult> NewSocialRole([FromServices] AdminUserManagement __AdminUserManagement,
                                                       [FromServices] SocialUserManagement __SocialUserManagement,
                                                       [FromServices] SessionAdminUserManagement __SessionAdminUserManagement,
                                                       [FromBody] ParserSocialUserRole __ParserModel,
                                                       [FromHeader(Name = "session_token_admin")] string SessionToken)
        {
            ...
            try {
                ...
                if (Error == ErrorCodes.USER_DOES_NOT_HAVE_PERMISSION) {
                    return Problem(400, RESPONSE_MESSAGES.NOT_ALLOW_TO_DO, new string[]{ "add new social role" });
                }
                #endregion

                #region Parser Social Role
                var Role = new SocialUserRole();
                ...
                (FindRole, Error) = await __SocialUserManagement.GetRole(Role.RoleName);
