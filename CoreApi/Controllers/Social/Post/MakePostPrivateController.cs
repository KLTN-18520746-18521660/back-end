...
namespace CoreApi.Controllers.Social.Post
{
    [ApiController]
    [Route("/api/post")]
    public class MakePostPrivateController : BaseController
    {
        public MakePostPrivateController(BaseConfig _BaseConfig) : base(_BaseConfig)
        {
        }

        [HttpPost("private/id/{post_id}")]
        ...
        public async Task<IActionResult> MakePostPrivate([FromServices] SessionSocialUserManagement __SessionSocialUserManagement,
                                                         ...
                                                         [FromHeader(Name = "session_token")] string SessionToken)
        {
            ...
            try {
                ...
                if (__SocialPostManagement.ValidateChangeStatusAction(Post.Status.Type, StatusType.Private) == ErrorCodes.INVALID_ACTION) {
                    return Problem(400, RESPONSE_MESSAGES.NOT_ALLOW_TO_DO, new string[]{ "make private post" });
                }
                #endregion

                Error = await __SocialPostManagement.MakePostPrivate(Post.Id, Post.Owner);
                if (Error != ErrorCodes.NO_ERROR) {
                    throw new Exception($"MakePostPrivate failed, ErrorCode: { Error }");
                }
