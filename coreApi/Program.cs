...
namespace coreApi
{
    ...
    public class Program
    {
            ...
            Host.CreateDefaultBuilder(args)
                .ConfigureLogging((context, loggerBuilder) => {
                    _loggerFactory = LoggerFactory.Create(_loggerBuilder => {
                        ...
                        loggerBuilder = _loggerBuilder;
                    });
                    
                    logFileFormat = context.Configuration.GetValue<string>("LogFileFormat");
                    if (logFileFormat == null) {
                        logFileFormat = coreApi.ConfigurationDefaultVariable.LOG_FILE_FORMAT;
                    }
                    _loggerFactory.AddFile(logFileFormat);
                })
                .ConfigureServices((context, service) => {
                    try {
                        ...
                        if (portConfig != null) {
                            ...
                            apiPort = coreApi.ConfigurationDefaultVariable.PORT;
                        }

                        certPath =  context.Configuration.GetSection("Certificate").GetValue<string>("Path");
                        if ((certPath == null || !System.IO.File.Exists(certPath)) && isHttps) {
                ...
                .ConfigureWebHostDefaults(webBuilder => {
                    webBuilder.UseKestrel(kestrelServerOptions => {
                        ...
                        kestrelServerOptions.Listen(System.Net.IPAddress.Any, apiPort, listenOptions => {
                            if (isHttps) {
                                // Config ssl pfx
                                listenOptions.UseHttps(certPath, coreApi.ConfigurationDefaultVariable.PASSWORD_CERTIFICATE);
                            }
                        });
                    });
        ...
        private static void LogStartInformation()
        {
            ...
            _logger.LogInformation("Using ssl: {isHttps}", isHttps);
            _logger.LogInformation("Logs folder: {logPath}", System.IO.Path.GetDirectoryName(logFileFormat));
            _logger.LogInformation("Temp folder: {tmpPath}", tmpPath);
        }
        ...
        public static void Main(string[] args)
        {
            ...
            try {
                ...
                _logger = _loggerFactory.CreateLogger<Program>();
                LogStartInformation();
                coreApi.Common.Logger.LogException(_logger, e.ToString());
            }
